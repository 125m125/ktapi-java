...
public interface KtRetrofitClient {
    ...
    Call<List<Item>> getItems(@Path("userid") String userid);
    ...
    Call<List<Payout>> getPayouts(@Path("userid") String userid);
    ...
    Call<WriteResult<Payout>> createPayout(@Path("userid") String userid, @Field("type") BUY_SELL type,
            @Field("item") String itemid, @Field("amount") int amount);
    ...
    Call<List<Trade>> getTrades(@Path("user") final String user);
