...
public class KtCachingRequesterIml implements KtRequester, NotificationListener, KtCachingRequester {
    ...
    @Override
    public boolean isValidMessageList(final String userid, final List<Message> messages) {
        return isValid(KtCachingRequesterIml.MESSAGES + userid, messages);
    }

    @Override
    public boolean isValidPayoutList(final String userid, final List<Payout> payouts) {
        return isValid(KtCachingRequesterIml.PAYOUTS + userid, payouts);
    }

    @Override
    public boolean isValidTradeList(final String userid, final List<Trade> trades) {
        return isValid(KtCachingRequesterIml.TRADES + userid, trades);
    }

    @Override
    public boolean isValidItemList(final String userid, final List<Item> items) {
    ...
    @Override
    public Result<PusherResult> authorizePusher(final String userid, final String channel_name, final String socketId) {
    ...
    protected class ExposedResult<T> extends Result<T> {
        @Override
